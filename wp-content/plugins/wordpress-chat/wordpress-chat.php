...
class WPMUDEV_Chat {
	...
	var $chat_current_version = '1.3.3';
	...
	var $using_popup_twitter_auth_template 	= false;
			...
			switch ($_GET['wpmudev-chat-action']) {
				...
				case 'pop-twitter':
					...
					if ($this->is_twitter_setup()) {
						...
						$this->using_popup_twitter_auth_template = true;
...
<?php */ ?>
					...
					<table border="0" cellpadding="4" cellspacing="0">
						...
						<tr>
							...
							<td>
								...
								<label for="chat_twitter_api_key"><?php _e('Consumer key', $this->translation_domain); ?></label><br />

								<input type="text" id="chat_twitter_api_key" name="chat_default[twitter_api_key]" value="<?php print $this->get_option('twitter_api_key', ''); ?>" class="" size="40" /><br/><br />

								<label for="chat_twitter_api_secret"><?php _e('Consumer secret', $this->translation_domain); ?></label><br />

								<input type="text" id="chat_twitter_api_secret" name="chat_default[twitter_api_secret]" value="<?php print $this->get_option('twitter_api_secret', ''); ?>" class="" size="40" />
								...
								<ol>
									...
									<li><?php _e('Fill in the form with values for your website:', $wpmudev_chat->translation_domain); ?>
										...
										<ul>
											...
											<li><?php _e('<strong>Callback URL</strong>: Should be the home URL of your website. <strong>Not the page where Chat is displayed</strong>', $wpmudev_chat->translation_domain); ?></li>
	...
	function wp_head() {
		...
		if (($this->get_option('twitter_api_key') != '') && ($this->get_option('twitter_api_secret') != '')) {
	...
	function wp_footer() {
		...
		if ($this->using_popup_twitter_auth_template) {

			wp_print_scripts( array('jquery', 'jquery-cookie', 'chat_js') );
...
//				echo "smessage=[". $smessage ."] after base64_decode<br />\r\n";



	   			$smessage = preg_replace(array('/<code>/','/<\/code>/'), array('[code]', '[/code]'), $smessage);

//				echo "smessage=[". $smessage ."] after preg_replace<br />\r\n";



				$smessage = strip_tags($smessage);

				$smessage = htmlentities($smessage);

//				echo "smessage=[". $smessage ."] after htmlentities<br />\r\n";



//				echo "smessage=[". $smessage ."] after strip_tags<br />\r\n";
