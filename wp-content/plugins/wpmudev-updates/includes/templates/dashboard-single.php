<section id="profile" class="grid_container free-user clearfix">
	<section class="contents clearfix">
		...
		<?php if (isset($local_projects[$project['id']])) { ?>
			<button class="cta"><i class="icon-ok"></i> <?php _e('Installed', 'wpmudev'); ?></button>
		<?php } else if ($this->auto_install_url($paid_project_id)) { ?>
			<button class="cta" data-href="<?php echo esc_url($this->auto_install_url($paid_project_id)); ?>"><i class="icon-download-alt"></i> <?php _e('Install', 'wpmudev'); ?></button>
		<?php } else { ?>
...
<section class="premium-content">
	<section class="layer">
		<section class="promotional">
			...
			<a class="btn" href="<?php echo apply_filters('wpmudev_upgrade_url', 'https://premium.wpmudev.org/membership/'); ?>">
