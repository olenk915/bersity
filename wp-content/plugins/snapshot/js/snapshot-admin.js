	...
	function snapshot_build_list_checkbox(table_type, table_set) {
		...
		if (table_set != undefined) {
			...
			for (var table_name in table_set) {
				...
				jQuery('ul#snapshot-table-list-'+table_type).append('<li><input id="snapshot-tables-'+table_name+'" '+table_checked+' class="snapshot-table-item" type="checkbox" name="backup_tables['+table_type+']['+table_name+']" value="'+table_name+'"> <label for="snapshot-tables-">'+table_name+'</label></li>');
			}
