...
if (!class_exists('WPMUDEVSnapshot')) {
	class WPMUDEVSnapshot {
		...
		function __construct() {
			...
			$this->DEBUG									= false;
			$this->_settings['SNAPSHOT_VERSION'] 			= '2.2';
		...
		function snapshot_admin_notices_proc($local_type = '', $local_message = '') {
			...
			//echo "_messages<pre>"; print_r($this->_messages); echo "</pre>";
			//exit;
		...
		function snapshot_ajax_backup_table($item, $_post_array) {
			...
			if (isset($_SESSION['snapshot']['backupItemFolder'])) {
				...
				if ($fp) {
					...
					/*
					if ($number_rows_segment != $table_data['rows_end']) {

						$error_status['errorStatus'] = true;
						$error_status['errorText'] = "<p>". __('ERROR: Snapshot encounters a mismatch on the backup of table ['. $table_data['table_name'].'] segment.', SNAPSHOT_I18N_DOMAIN) ."</p>";					
						$error_status['errorText'] .= "<p> number_rows_segment=[". $number_rows_segment ."] rows_end=[". $table_data['rows_end'] ."]</p>";
...
//die();
					//$error_status['DEBUG'] = "file_data_key=[". $file_data_key ."] files<pre>" . 
					//	print_r($_SESSION['snapshot']['files_data']['included'][$file_data_key], true) ."</pre>";
					//return $error_status;
		...
		function snapshot_ajax_show_blog_tables() {
			...
			$json_data = array();
			if ( (isset($_POST['snapshot_blog_id'])) && (intval($_POST['snapshot_blog_id'])) ) {
				$blog_id = intval($_POST['snapshot_blog_id']);
				$tables = snapshot_utility_get_database_tables(intval($blog_id));
				if ($tables) {
	...
	//			$config_data = get_option( 'snapshot_1.0');
		...
		function snapshot_backup_cron_proc($item_key) {
			...
			// Uncomment to write out debug log to 'uploads/snapshot/cron_log.txt' file
//			$fp_cron_debug = fopen(trailingslashit($this->_settings['backupBaseFolderFull']) .'cron_log.txt', 'a');
//			if ((isset($fp_cron_debug)) && ($fp_cron_debug))
//				fwrite($fp_cron_debug, 'Init '. __FUNCTION__ .': item_key:'. $item_key ."\r\n");
			 ...
			 || ($item['tables-option'] == "all") || ($item['tables-option'] == "selected") ) {
				...
				if ((!defined('SNAPSHOT_DOING_CRON')) || (SNAPSHOT_DOING_CRON != true)) {
					...
					if ((isset($item['interval'])) && ($item['interval'] == "")) {
						...
						if ((isset($fp_cron_debug)) && ($fp_cron_debug))
							fwrite($fp_cron_debug, 'wp_next_scheduled:'. $timestamp ."\r\n");
				...
				if ((isset($fp_cron_debug)) && ($fp_cron_debug))
					fwrite($fp_cron_debug, "stating backup:\r\n");
			...
			//echo "finished. Calling process_item_remote_files(".$item_key.")<br />";
			$this->process_item_remote_files($item_key);
			//echo "returned process_item_remote_files(".$item_key.")<br />";
		
			if ((isset($fp_cron_debug)) && ($fp_cron_debug))
				fclose($fp_cron_debug);
		}
	
		function process_item_remote_files($item_key, $_really_send = false) {
//			if ($_really_send !== true) return;
			...
			foreach($item['data'] as $data_item_key => $data_item) {
				...
				if ($snapshot_locker->is_locked()) {
					...
					if ((isset($item['destination-sync'])) && ($item['destination-sync'] == "mirror")) 
						$doing_message = __('Syncing to destination', SNAPSHOT_I18N_DOMAIN);
					else
						$doing_message = __('Sending to destination', SNAPSHOT_I18N_DOMAIN);
		...
		function process_item_send_archive($item, $data_item, $snapshot_locker) {
			...
			if ( (isset($data_item['destination-status'])) && (count($data_item['destination-status'])) ) {
				...
				if ( (isset($destination_status['sendFileStatus'])) && ($destination_status['sendFileStatus'] == true) ) {
					//echo "finished item[". $item_key ."] data_item[". $data_item."] file[". $data_item['filename'] ."]<br />";
					return;
				}
			...
			//echo "processing item[". $item_key ."] data_item[". $data_item_key ."] file[". $data_item['filename'] ."]<br />";
			...
			//echo "current_backupFolder=[". $current_backupFolder ."]<br />";
...
//				}
			...
			if ($data_item['destination-sync'] == "archive") {
				...
				$snapshot_logger->log_message("Sending file: ". basename($backupFile) ." ". snapshot_utility_size_format(filesize($backupFile)));
...
//				die();				
				...
				if (strlen($data_item['filename'])) {
					...
					$snapshot_logger->log_message("Sending file: ". basename($backupFile));
					$snapshot_logger->log_message("Destination: ". $destination['type'] .": ". stripslashes($destination['name']));
...
/*
			$fp_cron_debug = fopen(trailingslashit($this->_settings['backupBaseFolderFull']) .'cron_log.txt', 'a');			
			if ((isset($fp_cron_debug)) && ($fp_cron_debug)) {
				fwrite($fp_cron_debug, "errno=[". $errno ."]<br />\r\n");
				fwrite($fp_cron_debug, "errstr=[". $errstr ."]<br />\r\n");
				fwrite($fp_cron_debug, "errfile=[". $errfile ."]<br />\r\n");
				fwrite($fp_cron_debug, "errline=[". $errline ."]<br />\r\n");
			}
*/
