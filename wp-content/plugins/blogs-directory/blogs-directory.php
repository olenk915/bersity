				...
				if ( count($blogs) > 0 ) {
					...
					foreach ($blogs as $blog){
						...
						$content .= '<tr>';
							...
							$content .= '<a style="text-decoration:none; font-size:1.5em; margin-left:20px;" href="http://' . $blog['domain'] . $blog['path'] . '" target="_blank">' . $blog_title . '</a><br />';
	...
	if ( !empty( $phrase ) ) {
		...
		$content .= '<tr>';
			...
			$content .= '<td style="font-size:12px; text-align:right;" width="20%">';
				...
				$content .= '<input name="Submit" value="' . __('Search uBlogs','blogs-directory') . '" type="submit">';
